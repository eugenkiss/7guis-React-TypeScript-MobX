...
import {Application, Coord, EmptyFormula, Formula, Number, Range, Textual} from './formula'
...
export class FormulaParser {
  ...
  private expression(): Formula {
    switch(this.lookahead.token) {
      case CELL:
        ...
        this.nextToken()
        if (this.lookahead.token as any === COLON) { // Range
          this.nextToken()
  ...
  private application(): Formula {
    ...
    this.nextToken()
    if (this.lookahead.token !== OPEN_BRACKET)
      throw new Error('No opening bracket: ' + opName)
    this.nextToken()
    const args = new Array<Formula>()
    while (true) {
      if (this.lookahead.token as any === EPSILON)
        throw new Error('No closing bracket')
      args.push(this.expression())
      if (this.lookahead.token as any === COMMA) this.nextToken()
      if (this.lookahead.token as any === CLOSE_BRACKET)
